...
export default function LeaveReportPage() {
  ...
  const handleExportPDF = () => {
    try {
      ...
      let totAccrued = 0, totTaken = 0, totPending = 0, totRemaining = 0, totAccruedYear = 0, totTakenYear = 0;
      ...
      const tableData = filteredEmployees.map((emp, index) => {
        ...
        totAccrued += balance.totalAccrued;
        totTaken += balance.totalTaken;
        totPending += balance.pendingRequests;
        totRemaining += balance.remainingBalance;
        totAccruedYear += balance.accruedThisYear;
        totTakenYear += balance.takenThisYear;
      ...
      tableData.push([
        '', '', '', 'Total',
        totAccrued.toFixed(1),
        totTaken.toString(),
        totPending.toString(),
        totRemaining.toFixed(1),
        totAccruedYear.toFixed(1),
        totTakenYear.toString(),
        '', ''
      ]);

      autoTable(doc, { 
        head: [['Sr.', 'Emp ID', 'Employee Name', 'Department', 'Total Accrued', 'Total Used', 'Pending', 'Remaining', 'Accrued (Year)', 'Used (Year)', 'Accrual Rate', 'Next Accrual']], 
        body: tableData, 
        startY: 70,
        theme: 'grid',
        headStyles: { 
      ...
      if (finalY + 40 < doc.internal.pageSize.height) {
        ...
        const detailData = filteredEmployees.flatMap((emp, empIdx) => {
          const userLeaves = getEmployeeLeaveRequests(emp.id);
          const empIdFormatted = emp.employeeId || `EMP${String(emp.id).padStart(3, '0')}`;
          if (userLeaves.length === 0) return [];
          return userLeaves.map((leave: any, idx: number) => [
            empIdFormatted,
            `${emp.firstName} ${emp.lastName}`,
            leave.type || '-',
